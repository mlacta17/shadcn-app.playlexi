...
| Component | Description | Design Status | Implementation Status | Notes |
|-----------|-------------|---------------|----------------------|-------|
| **SignInDialog** | Navbar sign-in modal with OAuth | Done | Done | Modal triggered by navbar "Sign in" button. Shows Google + Apple OAuth buttons. "Sign up" link only appears when tutorial is not complete (`localStorage("playlexi_tutorial_complete")` not set) — routes to `/onboarding/tutorial?returnTo=/?signIn=true` (tutorial first, then back to dashboard with auto-open dialog). When tutorial is complete, the "Sign up" link is hidden (OAuth buttons handle both sign-in and sign-up). Located at `components/auth/sign-in-dialog.tsx`. Figma: node `3097:49442`. |

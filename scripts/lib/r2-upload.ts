...
/**
 * TTS audio types generated by OpenAI.
 ...
 *
 * @param word - The word (used for the filename)
 * @param type - The type of TTS audio
 * @returns The R2 key, e.g., "audio/tts/intros/castle.mp3"
 *
 * @example
 * ```typescript
 * getTtsAudioKey("castle", "intro")   // "audio/tts/intros/castle.mp3"
 * getTtsAudioKey("castle", "sentence") // "audio/tts/sentences/castle.mp3"
 * getTtsAudioKey("castle", "definition") // "audio/tts/definitions/castle.mp3"
 * ```
 */
export function getTtsAudioKey(word: string, type: TtsAudioType): string {
  // Sanitize the word for use as a filename
  const sanitized = word.toLowerCase().replace(/[^a-z0-9]/g, "_")
...
/**
 * Upload TTS audio for a word and return the public URL.
 *
 * @param word - The word
 * @param type - The type of TTS audio (intro, sentence, definition)
 * @param audioBuffer - The audio file content
 * @returns The public URL of the uploaded audio
 *
 * @example
 * ```typescript
 * const url = await uploadTtsAudio("castle", "intro", audioBuffer)
 * // Returns: "/api/assets/audio/tts/intros/castle.mp3"
 * ```
 */
export async function uploadTtsAudio(
  ...
  const key = getTtsAudioKey(word, type)
  await uploadToR2(key, audioBuffer, "audio/mpeg")
  return getPublicUrl(key)
}

/**
 * Check if TTS audio exists for a word.
 *
 * @param word - The word to check
 * @param type - The type of TTS audio
 * @returns true if the audio exists in R2
 */
